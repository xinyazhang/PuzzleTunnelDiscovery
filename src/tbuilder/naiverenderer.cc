...
// FIXME: Tons of dup code from Naive2DRenderer
//        The only difference is Private::create_cube
struct Naive3DRenderer::Private {
	GLFWwindow *window;
			...
			Eigen::MatrixXi& cubei)
	{
		...
		double xmin = mins(0), ymin = mins(1), zmin = mins(2);
		double xmax = maxs(0), ymax = maxs(1), zmax = maxs(2);
		cubev.resize(8, 3);
...
Naive3DRenderer::Naive3DRenderer()
	:p_(new Naive3DRenderer::Private)
{
}
...
void Naive3DRenderer::addSplit(const Eigen::VectorXd& center, 
		      const Eigen::VectorXd& mins,
		      const Eigen::VectorXd& maxs)
{
...
void Naive3DRenderer::addCertain(const Eigen::VectorXd& center, 
			const Eigen::VectorXd& mins,
			const Eigen::VectorXd& maxs,
			bool isfree)
{
...
void Naive3DRenderer::addLine(const Eigen::MatrixXd& LV)
{
	std::lock_guard<std::mutex> guard(p_->mutex);
...
int Naive3DRenderer::addDynamicLine(const Eigen::MatrixXd& LV)
{
	std::lock_guard<std::mutex> guard(p_->mutex);
...
void Naive3DRenderer::removeDynamicLine(int token)
{
	std::lock_guard<std::mutex> guard(p_->mutex);
...
void Naive3DRenderer::init()
{
	p_->window = init_glefw(1440, 900, "Naive Renderer");
	p_->gui.reset(new GUI(p_->window));
}

void Naive3DRenderer::launch_worker(std::function<int(NaiveRenderer*)> fn)
{
	p_->worker.reset(new std::thread(fn, this));
}

void Naive3DRenderer::setEnv(const Geo* env)
{
	p_->env = *env;
}

void Naive3DRenderer::workerReady()
{
	p_->mutex.unlock();
}
